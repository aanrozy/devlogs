<script lang="ts">
	...
	class Particle {
		...
		constructor(x: number, y: number) {
			...
			this.baseOpacity = Math.random() * 0.3 + 0.1; // 0.1 to 0.4
			
			// Make about 40% of particles blink like fireflies
			this.isBlinking = Math.random() < 0.4;
		...
		draw() {
			...
			if (this.isBlinking) {
				// Sine wave pulse
				const pulse = Math.sin(this.blinkPhase); // -1 to +1
				currentOpacity = this.baseOpacity + (pulse * this.baseOpacity * 0.8);
			...
			// Add a slight glow effect for blinking particles when bright
			if (this.isBlinking && currentOpacity > this.baseOpacity * 1.2) {
				ctx.fillStyle = `rgba(254, 240, 138, ${currentOpacity * 0.4})`;
				ctx.beginPath();
